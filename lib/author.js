...
Then run: agent-lens author --apply agent-lens.authored.json
`;
}
...
function applyAuthoring(state, authored, opts = {}) {
  ...
  if (authored.summary) {
    const h = clean(authored.summary.headline, 220);
    const s = clean(authored.summary.subhead, 500);
    if (h) state.summary.headline = h;
    if (s) state.summary.subhead = s;
  }
