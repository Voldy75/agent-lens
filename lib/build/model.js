...
function buildHealth(files, modules, gitData, deps) {
  const rewrites = Object.entries(gitData.churn)
    .filter(([p]) => files.some((f) => f.path === p && !f.test))
  ...
  const rows = rewrites.map(([p, c]) => {
    ...
    return {
      where: p,
      rewrites: c.commits,
      tests: tested ? `${mod._tests} test files` : null,
      meaning: c.commits >= 8
  ...
  return {
    stats: [
      ...
      { label: 'Source files', value: String(files.filter((f) => !f.test).length), note: `${files.filter((f) => f.test).length} test files` },
...
function buildCost(sessions, modules) {
  if (!sessions.available) return { available: false };
  const total = sessions.input + sessions.output;
  ...
  for (const [file, n] of Object.entries(sessions.touched)) {
    const rel = file.replace(/^.*?\/(?=src\/|app\/|lib\/)/, '');
    const m = modules.find((x) => [...x._paths].some((p) => rel.endsWith(p)));
    if (m) byMod.set(m.name, (byMod.get(m.name) || 0) + n);
  }
  const byArea = [...byMod.entries()].sort((a, b) => b[1] - a[1]).slice(0, 6)
    .map(([label, value]) => ({ label, value, display: `${value} edits` }));

  return {
    available: true,
    stats: [
      { label: 'Sessions', value: String(sessions.sessions), note: sessions.agents.join(' + ') },
      { label: 'Tokens', value: M(total), note: 'in + out combined' },
      { label: 'Messages', value: String(sessions.messages), note: 'model turns recorded' },
      { label: 'Per 1M tokens', value: `${(sessions.messages / Math.max(1, total / 1e6)).toFixed(0)} turns`, note: 'turn density' }
    ],
    seriesLabel: 'Tokens per week',
    series,
    byArea,
    caveat: 'Tokens are spent per message, not per file. The split above is attributed by which files a session touched — treat it as a rough indication, not an invoice. No pricing is applied because rates differ by model and plan.'
  };
}
