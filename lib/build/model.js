...
function buildModules(files, gitData) {
  ...
  for (const f of files) {
    ...
    for (const spec of f.imports) {
      const t = resolveImport(f.path, spec, fileSet, files.resolver);
...
function buildEdges(files, modules) {
  ...
  for (const f of files) {
    ...
    if (!from) continue;
    for (const spec of f.imports) {
      const target = resolveImport(f.path, spec, fileSet, files.resolver);
      if (!target) continue;
...
function buildHealth(files, modules, gitData, deps) {
  ...
  // Many agent-built projects have no automated tests at all, often on
  // purpose. "100% untested" is alarming and says nothing per module then;
  // say it once, plainly, and stop repeating it on every row.
  const testFiles = files.filter((f) => f.test).length;
  const noTests = testFiles === 0;
  ...
  const rows = rewrites.map(([p, c]) => {
    ...
    return {
      where: p,
      rewrites: c.commits,
      tests: tested ? `${mod._tests} test file${mod._tests === 1 ? '' : 's'}` : noTests ? '—' : null,
      meaning: c.commits >= 8
        ...
        : tested
          ...
          : noTests
            ? (c.commits >= 3 ? 'Reworked several times. Worth a look before you build on top of it.' : 'Changed twice.')
            : c.commits >= 3
  ...
  return {
    stats: [
      ...
      noTests
        ? { label: 'Automated tests', value: 'None', note: 'this project has no test files, so nothing checks changes automatically' }
      ...
      { label: 'Source files', value: String(files.filter((f) => !f.test).length), note: noTests ? 'no test files' : `${testFiles} test file${testFiles === 1 ? '' : 's'}` },
