...
function subhead(gitData, sessions, plans, health) {
  const bits = [];
  if (plans.planFile) bits.push(`Plan read from ${plans.planFile}.`);
  ...
  if (sessions.available) bits.push(`${sessions.sessions} agent sessions found for this folder (${sessions.agents.join(', ')}).`);
  else bits.push('No agent session logs found, so there is no cost figure.');
  return bits.join(' ');
}
...
async function buildState(root, opts = {}) {
  ...
  const plans = collectPlans(root, { planFile: opts.planFile, gitChurn: gitData.churn });
  if (!plans.steps.length) {
    const n = (plans.filesSeen || []).length;
    warnings.push(n
      ? `read ${n} markdown file${n === 1 ? '' : 's'}, none of them a plan — run "agent-lens author --prompt" to have your agent write one, or point at a file with --plan <path>`
  ...
  (sessions.reasons || []).forEach((r) => warnings.push(r));
  ...
  const state = {
    ...
    summary: { headline: headline(plan, health, gitData, cost), subhead: subhead(gitData, sessions, plans, health) },
    headings: {
      plan: {
    ...
    meta: {
      ...
      agentsDetected: normaliseAgents([...(plans.agents || []), ...(sessions.agents || []), ...(sessions.detected || [])]),
